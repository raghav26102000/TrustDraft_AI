...
import { ocrPdfFallback } from '@/lib/ai/ocrFallback'
...
async function parsePdfInner(filePath) {
  ...
  const tooShort = text.length < MIN_USEFUL_TEXT_CHARS
  const alphaRatio = text.length > 0 ? (text.match(/[a-zA-Z]/g) || []).length / text.length : 0
  const looksGarbled = !tooShort && alphaRatio < 0.4

  if (tooShort || looksGarbled) {
    // Attempt OCR fallback before giving up. This is slow — only triggered
    // when normal extraction genuinely failed or looks unreliable.
    try {
      const ocrText = await withTimeout(ocrPdfFallback(filePath), OCR_TIMEOUT_MS, 'pdf ocr')
      return ocrText
    } catch (ocrErr) {
      if (tooShort) {
        throw new Error('PDF appears empty or only contains scanned images, and OCR fallback also failed: ' + ocrErr.message)
      }
      throw new Error('PDF text extraction looks unreliable, and OCR fallback also failed: ' + ocrErr.message)
    }
  }
