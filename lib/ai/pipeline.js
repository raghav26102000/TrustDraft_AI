...
import { semanticChunk } from '@/lib/ai/semanticChunk'
...
async function runCore(submissionId) {
  ...
  for (const doc of supporting) {
    try {
      ...
      const regexChunks = chunkText(text)
      const expectedMinChunks = Math.max(3, Math.floor(text.length / 1500))
      let chunks = regexChunks
      let chunkMethod = 'regex'
      if (regexChunks.length < expectedMinChunks) {
        warn(
          submissionId,
          `regex chunking produced only ${regexChunks.length} chunks for ${doc.original_name} (expected ≥${expectedMinChunks}), falling back to semantic chunking`,
        )
        chunks = await semanticChunk(text)
        chunkMethod = 'semantic'
      }
      for (const c of chunks) allChunks.push({ sourceDoc: doc.original_name, text: c })
      log(submissionId, `parsed supporting: ${doc.original_name} → ${chunks.length} chunks (${chunkMethod})`)
    } catch (err) {
